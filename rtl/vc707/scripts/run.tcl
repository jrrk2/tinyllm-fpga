...
# patchable); INIT baked from the firmware by bin2init.py -> the .svh named by
# PROGMEM_INIT.  PROGMEM_RAMB selects the primitive over the inferred $readmemh
# path (sim-only).  Lets `make fw-update` swap firmware in the built .bit.
set _pminit [expr {[info exists ::env(PROGMEM_INIT_FILE)] ? $::env(PROGMEM_INIT_FILE) : [file normalize src/soc/progmem_init_shell.svh]}]
...
set _defs [list "PICORV32_REGS=picosoc_regs" "PROGMEM_HEX=\"$_fw_mem\"" \
                "PROGMEM_RAMB" "PROGMEM_INIT=\"$_pminit\""]
...
if {$_picosoc_engine} {
    ...
    # progmem is a DIRECTLY instantiated RAMB16_S36_S36 (forced BRAM, updatemem-
    # patchable); INIT baked from the firmware by bin2init.py -> the .svh named by
    # PROGMEM_INIT.  PROGMEM_RAMB selects the primitive over the inferred
    # $readmemh path (sim-only; it collapses to LUTROM in synth).
    set _pminit [expr {[info exists ::env(PROGMEM_INIT_FILE)] ? $::env(PROGMEM_INIT_FILE) : [file normalize src/soc/progmem_init.svh]}]
    ...
    puts "INFO: PICOSOC_ENGINE — SoC front-end (ILA=$_eila, fw=$_fwmem, progmem=RAMB16 init=$_pminit)"
