...
# patchable).  Bake its INIT from the firmware .bin into src/soc/progmem_init.svh
# (a literal `include in progmem_bram.v, found via the src/soc include dir).
# PROGMEM_RAMB selects the primitive over the inferred $readmemh sim path.
regsub {firmware_([^/]*)\.mem$} $_fw_mem {fw_\1.bin} _fw_bin
exec -ignorestderr python3 -E src/soc/bin2init.py $_fw_bin src/soc
puts "INFO: baked progmem_init_lane{0..3}.svh from $_fw_bin"
...
set _defs [list "PICORV32_REGS=picosoc_regs" "PROGMEM_HEX=\"$_fw_mem\"" "PROGMEM_RAMB"]
...
if {$_picosoc_engine} {
    ...
    # progmem is a DIRECTLY instantiated RAMB16_S36_S36 (forced BRAM, updatemem-
    # patchable).  Bake its INIT from the firmware .bin into src/soc/progmem_init.svh
    # (literal `include in progmem_bram.v).  PROGMEM_RAMB selects the primitive over
    # the inferred $readmemh path (sim-only; it collapses to LUTROM in synth).
    regsub {firmware_([^/]*)\.mem$} $_fwmem {fw_\1.bin} _fwbin
    exec -ignorestderr python3 -E src/soc/bin2init.py $_fwbin src/soc
    puts "INFO: baked progmem_init_lane{0..3}.svh from $_fwbin"
